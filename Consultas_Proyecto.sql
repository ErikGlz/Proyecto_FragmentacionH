...
--Las consultas se hicieron desde la instancia del esquema SalesAW
...
@cantidad int, @producto int, @orden int
as
begin
set nocount on;
declare @registrosActualizados int
...
go
SELECT * FROM Sales.SalesOrderDetail --Para verificar ID's de SalesOrderID válidos
go

--Vista:
create view vista_e as
select s.SalesOrderID as Orden, s.ProductID as IDProd, s.OrderQty as Cantidad, p.Name as Nombre
from [LOCALHOST\SERVER1].SalesAW.Sales.SalesOrderDetail s
...
go
select SalesOrderID from Sales.SalesOrderHeader--Para verificar ID's de SalesOrderID válidos
go
...
@customerID int, @newEmail nvarchar(50) 
as
begin
...
begin
	...
	where BusinessEntityID = (select PersonID from Sales.Customer
						      where CustomerID = @customerID)
end
end
go	
exec consulta_g 11000,'nuevocorreo@gmail.com'
select * from Sales.Customer where PersonID is not null--Para checar valores validos de customerID
go
